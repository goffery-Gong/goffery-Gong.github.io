...
![](http://wx1.sinaimg.cn/large/d8b81fbfly1g1920jp28wj22c02b9u0x.jpg)
...
1. 程序装入与链接
   ...
   编译，链接（静态链接，装入时动态链接，运行时动态链接），装入（绝对装入，静态重定位，动态重定位）

2. 地址空间

   **逻辑地址空间**：编译后每个目标模块从0开始编址（逻辑地址）；链接程序将各模块逻辑地址构成统一的逻辑地址空间。

   **物理地址空间**：内存中物理单元的集合；地址转换的最终地址。

   地址重定位：装入程序将代码装入内存时，将逻辑地址转换为物理地址。
